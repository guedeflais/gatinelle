...
export async function POST(request: Request) {
  ...
  if (user.accountType !== "COMMERCANT") {
    return corsJson(
  ...
  try {
    const transaction = await requestConversion(user.id, eurosToCents(parsed.data.amountEuros));
    ...
    return corsJson({ error: "Erreur lors de la demande de reconversion." }, { status: 500 });
  }
}
